# ============================================================================
# GLACIER SEGMENTATION - PHASE 1 OPTIMIZED (TRAIN V3)
# Direct 512×512 training with MixUp + Lovász-Softmax + Strong Dropout
# ============================================================================
...
# ============================================================================
# CONFIGURATION - PHASE 1 OPTIMIZED
# ============================================================================
...
CLASS_WEIGHTS = torch.tensor([0.8, 2.0, 4.0, 8.0], dtype=torch.float32)
...
# Loss configuration - Lovász-Softmax
USE_LOVASZ = True
...
def lovasz_softmax_from_logits(logits, labels, classes='present', per_image=False, ignore=None):
    """
    Lovász-Softmax wrapper for patch classification.
    Accepts:
      - logits: (B, C) - raw model outputs
      - labels: (B,) - integer class per patch
    Internally reshapes to (B, C, 1, 1) and (B, 1, 1).
    """
    probas = torch.softmax(logits, dim=1)  # (B, C)
    probas = probas.view(probas.size(0), probas.size(1), 1, 1)  # (B, C, 1, 1)
    labels = labels.view(labels.size(0), 1, 1)  # (B, 1, 1)
    return lovasz_softmax(probas, labels, classes=classes, per_image=per_image, ignore=ignore)

def lovasz_softmax(probas, labels, classes='present', per_image=False, ignore=None):
    """
    Lovász-Softmax loss for multi-class segmentation; here H=W=1 for patches.
    probas: (B, C, H, W)
    ...
    if per_image:
        ...
        for prob, lab in zip(probas, labels):
            loss = lovasz_softmax_flat(*flatten_probas(prob.unsqueeze(0), lab.unsqueeze(0)),
                                       classes=classes, ignore=ignore)
            losses.append(loss)
        return mean(losses)
    else:
        return lovasz_softmax_flat(*flatten_probas(probas, labels),
                                   classes=classes, ignore=ignore)

def lovasz_softmax_flat(probas, labels, classes='present', ignore=None):
    """
    probas: (N, C) after flatten
    labels: (N,)
    """
    if probas.numel() == 0:
    ...
    C = probas.size(1)
    losses = []
    # classes = 'present' means only classes present in labels
    class_range = range(C) if classes in ['all', 'present'] else classes
    ...
    for c in class_range:
        ...
        if (classes == 'present') and (fg.sum() == 0):
            continue
        pc = probas[:, c]  # (N,)
        errors = (1 - pc) * fg + pc * (1 - fg)  # hinge-like
        errors_sorted, perm = torch.sort(errors, descending=True)
        fg_sorted = fg[perm]
        # Lovász gradient
        gts = fg_sorted.sum()
        intersection = gts - torch.cumsum(fg_sorted, dim=0)
        union = gts + torch.cumsum(1 - fg_sorted, dim=0)
        jaccard = 1. - intersection / torch.clamp(union, min=1.)
        if errors_sorted.numel() > 1:
...
def flatten_probas(probas, labels):
    """
    Convert (B, C, H, W) and (B, H, W) into flat (N, C) and (N,)
    Here H=W=1 for patches, but works for any H, W.
    """
    B, C, H, W = probas.shape
    probas = probas.permute(0, 2, 3, 1).contiguous().view(-1, C)  # (N, C)
    labels = labels.view(-1)  # (N,)
    return probas, labels

def mean(ts):
    return sum(ts) / len(ts) if len(ts) else 0.0


# ============================================================================
# IMPROVED PATCH CNN - STRONGER DROPOUT
# ============================================================================
...
class OptimizedGlacierDataset(Dataset):
    def __init__(self, img_ids, train_dir, patch_size=64, stride=16, augment=False):
    ...
    def augment_patch(self, patch):
        ...
        if np.random.rand() > 0.6:
            ...
            patch = torch.roll(patch, (shift_x, shift_y), dims=(1, 2))
...
def train_epoch(model, loader, optimizer, criterion, device, use_mixup=False, mixup_alpha=0.2):
    model.train()
    total_loss = 0.0
    all_preds = []
    all_labels = []
    ...
    for patches, labels in pbar:
        ...
        # MixUp
        if use_mixup:
            patches, labels_a, labels_b, lam = mixup_data(patches, labels, mixup_alpha)
            optimizer.zero_grad(set_to_none=True)
            outputs = model(patches)  # logits (B, C)

            if USE_LOVASZ:
                loss_a = lovasz_softmax_from_logits(outputs, labels_a)
                loss_b = lovasz_softmax_from_logits(outputs, labels_b)
            ...
            if USE_LOVASZ:
                loss = lovasz_softmax_from_logits(outputs, labels)
            else:
...
def main():
    ...
    print(f"\n{'='*80}")
    print("GLACIER SEGMENTATION ")
    print(f"{'='*80}")
    print(f"✓ Device: {DEVICE}")
    print(f"✓ Image size: {IMAGE_SIZE}×{IMAGE_SIZE} (DIRECT, NO UPSCALING)")
    print(f"✓ Patch size: {PATCH_SIZE}×{PATCH_SIZE}")
    print(f"✓ Stride: {PATCH_STRIDE}")
    print(f"✓ MixUp augmentation: ENABLED (alpha={MIXUP_ALPHA})")
    print(f"✓ Lovász-Softmax loss: ENABLED")
    print(f"✓ Strong dropout: 0.4-0.6\n")
    ...
    train_dataset = OptimizedGlacierDataset(train_ids, args.train_data, PATCH_SIZE, PATCH_STRIDE, augment=True)
    print(f"✓ {len(train_dataset)} training patches\n")
    
    print("Preparing validation patches...")
    val_dataset = OptimizedGlacierDataset(val_ids, args.train_data, PATCH_SIZE, PATCH_STRIDE, augment=False)
    ...
    # Optimizer
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE_WARMUP, weight_decay=5e-5, betas=(0.9, 0.999))
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if USE_LOVASZ:
            train_loss, train_acc, train_mcc = train_epoch(model, train_loader, optimizer, 
                                                           criterion, DEVICE, USE_MIXUP, MIXUP_ALPHA)
